...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        # uncomment when we need the input ids
        # yield input_ids.cpu()
        ...
        if generation_config.num_beams > 1:
            raise ValueError(
                "streaming cannot be used with beam search (yet!). Make sure that `num_beams` is set to 1."
            )

        generation_mode = self._get_generation_mode(generation_config, assistant_model)
        if generation_mode == GenerationMode.BEAM_SEARCH:
            raise ValueError("Beam Search Generation not supported for streaming")
        elif generation_mode == GenerationMode.BEAM_SAMPLE:
            raise ValueError("Beam Sample Generation not supported for streaming")
        elif generation_mode == GenerationMode.GROUP_BEAM_SEARCH:
            raise ValueError("Group Beam Generation not supported for streaming")
        elif generation_mode == GenerationMode.CONSTRAINED_BEAM_SEARCH:
            raise ValueError("Constrained Beam Search Generation not supported for streaming")
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            yield from self.assisted_decoding(
                input_ids,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
            ...
            yield from self.contrastive_search(
                input_ids,
